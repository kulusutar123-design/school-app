...
portal_param = st.query_params.get("portal", "master")

default_idx = 0
if portal_param == "school":
    default_idx = 1
...
menu = st.sidebar.selectbox("🎯 Navigation Menu", ["Master Login", "School Login", "Student Login"], index=default_idx)
...
if menu == "Master Login":
    ...
    if not st.session_state.get('master_logged', False):
        ...
        with tab1:
            ...
            st.success("🔗 **Share Direct School Login Link:** `?portal=school` (ଆପଣଙ୍କ ସାଇଟ୍ ଲିଙ୍କ୍ ଶେଷରେ ଏହାକୁ ଯୋଡ଼ି ସ୍କୁଲ୍ କୁ ଦିଅନ୍ତୁ)")
            ...
            if not schools_db:
                ...
                for s_id, s_info in list(schools_db.items()):
                    ...
                    # MASTER CAN DELETE SCHOOLS (Added back as requested)
...
# ----------------- SCHOOL LOGIN (WITH CAPTCHA) -----------------
    ...
    if 'school_logged_id' not in st.session_state:
        ...
        st.markdown("---")
        st.success("🔗 **Share Direct Student Login Link:** `?portal=student` (ଆପଣଙ୍କ ସାଇଟ୍ ଲିଙ୍କ୍ ଶେଷରେ ଏହାକୁ ଯୋଡ଼ି ପିଲାମାନଙ୍କୁ ଦିଅନ୍ତୁ)")
            ...
            if school_students:
                ...
                for r_no, s_info in school_students.items():
                    if search_query.lower() in r_no.lower() or search_query.lower() in s_info['name'].lower() or search_query == "":
                        ...
                        # SCHOOL CAN DELETE STUDENTS (Added back as requested)
