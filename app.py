...
def get_db_connection():
    ...
    if not c.fetchone():
        def_m = {"username": "master", "password": hash_password("master123"), "email": "admin@school.com", "phone": "9999999999", "upi_id": "school@sbi", "reg_fee": 150.0, "gst_percent": 18.0}
...
if menu == "Home Page":
    ...
    notice_html = (
        ...
        "<div class='notice-item'>⏩ Online Student Registration Portal with 2-Step Verification is LIVE! <span class='new-badge'>NEW!</span></div>"
        "<div class='notice-item'>⏩ Auto-Refund Process Enabled for Rejected Applications!</div>"
        ...
        if temp_obj:
            ...
            if pay_mode == "Online Payment (UPI/QR)":
                ...
                with col_form:
                    ...
                    if st.button("Verify & Submit Final Application", type="primary"):
                        if not txn_id or len(txn_id) < 8:
                            ...
                            save_student(reg_data['school_sel'], reg_data['reg_id'], reg_data['data'], status="Pending_Master")
                ...
                if st.button("Submit Final Application", type="primary"):
                    ...
                    save_student(reg_data['school_sel'], reg_data['reg_id'], reg_data['data'], status="Pending_Master")
                    st.success(f"✅ Application Submitted Successfully! Your Registration ID is **{reg_data['reg_id']}**. It is now pending payment verification.")
                    st.session_state['payment_step'] = False
                    st.session_state['temp_student_data'] = None
...
# ----------------- MASTER LOGIN -----------------
    ...
    if not st.session_state.get('master_logged', False):
        ...
        with tab_pay:
            ...
            pending_payments = get_all_students_by_status("Pending_Master")
            
            if pending_payments:
                for s_id, r_no, p_st in pending_payments:
                    st.markdown(f"""
...
# ----------------- SCHOOL LOGIN -----------------
    ...
    if 'school_logged_id' not in st.session_state:
        ...
        pending_students = get_students_by_school(cur_school, status="Pending_School")
        ...
        with tab_approve:
            ...
            st.info("Students whose payments are verified by Master will appear here. Verify their details/marks, and Approve.")
            ...
            if pending_students:
                ...
                st.success(f"💳 **Master Verified Payment:** {pay_status}")
                ...
                with c_act1:
                    if st.button("✅ Approve Student & Save Marks", type="primary"):
                        ...
                        st.rerun()
                with c_act2:
                    if st.button("🚫 Reject (Auto Refund)"):
                        p_st['payment_mode'] = p_st.get('payment_mode', '') + " - [REFUND INITIATED]"
                        save_student(cur_school, app_roll, p_st, status="Rejected_Refund")
...
# ----------------- ZERO-CRASH RESULTS PORTAL (SUPPORTS 1 CRORE+ USERS) -----------------
    ...
    st.info(
        "🔗 **English:** Search using your Roll Number or Name. \n\n"
        "🔗 **हिन्दी:** कृपया अपना रोल नंबर या नाम दर्ज करके खोजें। \n\n"
        "🔗 **ଓଡ଼ିଆ:** କେବଳ Roll Number କିମ୍ବା Name ଦେଇ ସର୍ଚ୍ଚ କରନ୍ତୁ।"
    )
