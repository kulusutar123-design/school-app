...
# ==========================================
# 🔒 HIGH-SPEED DATABASE & SECURITY ARCHITECTURE
# ==========================================
...
# SQLite Database Connection (Supports 1 Crore+ Users with WAL Mode)
def init_db():
    conn = sqlite3.connect('school_enterprise.db', check_same_thread=False)
    conn.execute('PRAGMA journal_mode=WAL;') # Prevents DB lock/crash
    c = conn.cursor()
...
def save_school(s_id, data):
    conn.cursor().execute("REPLACE INTO schools (id, data) VALUES (?, ?)", (s_id, json.dumps(data)))
    conn.commit()
...
def save_student(s_id, roll_no, data):
    ...
    conn.cursor().execute("REPLACE INTO students (roll_no, dob, school_id, name, data) VALUES (?, ?, ?, ?, ?)",
                          (roll_no, n_dob, s_id, n_name, json.dumps(data)))
    conn.commit()
...
def auto_translate(text, lang_name):
    ...
    LC = {"Odia":"or", "Hindi":"hi", "Bengali":"bn", "Telugu":"te", "Tamil":"ta", "Marathi":"mr", "Gujarati":"gu", "Assamese":"as", "Kannada":"kn", "Malayalam":"ml", "Punjabi":"pa"}
...
def t(eng_text, lang):
    tr = {
        "School Portal": {"Odia": "ସ୍କୁଲ୍ ପୋର୍ଟାଲ୍", "Hindi": "स्कूल पोर्टल", "Bengali": "স্কুল পোর্টাল", "Marathi": "शाळा पोर्टल"},
        ...
        "NAME": {"Odia": "ନାମ", "Hindi": "नाम", "Bengali": "নাম"},
...
def generate_result_card_html(school_name_en, school_name_loc, st_data, roll_no, s_lang):
    ...
    s_name_en, m_name_en, f_name_en = st_data.get('name', '').upper(), st_data.get('mother_name', '').upper(), st_data.get('father_name', '').upper()
    ...
    qr_text = f"SCHOOL: {school_name_en} | NAME: {s_name_en} | ROLL: {roll_no} | DOB: {disp_dob} | MARKS: {tot_obt}/{st_data.get('total_full', 0)} | GRADE: {st_data.get('grade', '')} | RESULT: {st_data.get('result', '')}"
...
if menu != "Home Page" and menu != "New School Registration" and menu != "Master Login" and menu != "School Login" and menu != "Results":
...
# --- REGISTRATION ---
    ...
    with st.form("reg_form"):
        ...
        if st.form_submit_button("Submit Registration"):
            if not r_id or not r_name or not pwd: st.error("Fill mandatory fields (*)")
            elif pwd != cpwd: st.error("Passwords do not match!")
            elif r_id in schools_db: st.error("ID exists!")
            else:
                save_school(sanitize(r_id), {
...
# --- MASTER LOGIN ---
    ...
    if not st.session_state.get('master_logged', False):
        ...
        if st.button("Login"):
            if m_u == master_db["username"] and hash_password(m_p) == master_db["password"]:
        ...
        with t2:
            ...
            if sel_s != "--Select--":
                sts = get_students(sel_s)
                if sts:
                    e_roll = st.selectbox("Select Roll No", list(sts.keys()))
                    curr_st = sts[e_roll]
                    st.write(f"Currently editing Roll No: {e_roll} | Name: {curr_st.get('name','')}")
...
# --- SCHOOL LOGIN ---
    ...
    if 'school_logged_id' not in st.session_state:
        ...
        if st.button("Login"):
            if s_id in schools_db and schools_db[s_id]["pass"] == hash_password(s_pass):
                if schools_db[s_id].get("status", "Active") == "Active":
                    st.session_state['school_logged_id'] = s_id; st.rerun()
                else: st.error("Account Pending/Inactive.")
            else: st.error("Invalid Credentials!")
    else:
        c_id = st.session_state['school_logged_id']
        s_lang = schools_db[c_id].get("lang", "English")
        if st.button("Logout"): del st.session_state['school_logged_id']; st.rerun()
        st.info(f"🏫 Logged in as: {schools_db[c_id]['name']}")
        
        t1, t2, t3 = st.tabs(["📋 My Students", "➕ Add/Edit Student", "🖨️ Report Card"])
        cur_students = get_students(c_id)
        ...
        with t3:
            if cur_students:
                ...
                html_c = generate_result_card_html(schools_db[c_id]['name'], schools_db[c_id].get('name_local',''), cur_students[sel_r], sel_r, s_lang)
                st.markdown(html_c, unsafe_allow_html=True)
                create_pdf(f"res_{sel_r}.pdf", schools_db[c_id]['name'], cur_students[sel_r], sel_r)
                with open(f"res_{sel_r}.pdf", "rb") as f: st.download_button("Download PDF", f, file_name=f"Result_{sel_r}.pdf")

# --- RESULTS PORTAL (HIGH CONCURRENCY 1-CRORE SPEED) ---
elif menu == "Results":
    ...
    if st.button("View Result"):
        if s_q and d_q:
            sq_low = s_q.strip().lower()
            ndob = normalize_dob(d_q)
            c = conn.cursor()
            # O(1) Quick Search using SQLite Index
            c.execute("SELECT school_id, roll_no, data FROM students WHERE (roll_no=? OR name=?) AND dob=?", (s_q, sq_low, ndob))
            row = c.fetchone()
            
            if row:
                s_id, r_no, s_data = row[0], row[1], json.loads(row[2])
                sch = schools_db.get(s_id, {})
            ...
            else: st.error("❌ କୌଣସି ରେକର୍ଡ ମିଳିଲା ନାହିଁ! ଭୁଲ୍ ତଥ୍ୟ (Roll Number/Name କିମ୍ବା DOB) ଦେଇଛନ୍ତି।")
